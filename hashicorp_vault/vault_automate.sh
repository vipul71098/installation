...
if [ "$gitlab_response" == "[]" ];then
   echo "\n No user exists in gitlab.thewitslab.com with username : $user_name \n"
else
   ####################Create Policy########################################################################
    my_list=()
    ...
    python3 mutliple_path_policy.py "${my_list[@]}"
...
if [ "$status" != "null" ]; then
    ...
    python3 update_password_sheet.py $user_name $PASSWORD
